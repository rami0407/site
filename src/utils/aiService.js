...
חמש התחנות ומשפטי המעבר המחייבים:
1. [ מ ] משיכה וסקרנות (עירור עניין וידע קודם):
   - גירוי מוחשי (תמונה, חידה, דילמה יומיומית).
   - שאלת התלמיד: "מה מעורר בי סקרנות? ומה אנחנו רוצים לגלות?".
   - דוגמה להכלה והשתלבות, דוגמה להוראה דיפרנציאלית, דוגמה להערכה דיאגנוסטית.
   - משפט מעבר מחייב: "מתוך מה שהעליתם, ננסה להבין היום..."

2. [ פ ] פיתוח הבנה (המשגה ומידול):
   - מטרת השיעור ומדדי הצלחה תלת-ממדיים (תוכן, מיומנות, השתתפות), מילון מושגים, ומידול המורה (I Do) בחשיבה בקול.
   - שאלת התלמיד: "מה אנחנו לומדים? ואיך אסביר זאת במילים שלי?".
   - דוגמה להכלה (ייצוגי UDL, מילון מונחים), דיפרנציאליות (פיגומים Scaffolding), הערכה (בדיקת הבנה מהירה).
   - משפט מעבר מחייב: "הכרנו את הרעיון; כעת נבדוק איך הוא עובד ולמה"

3. [ ת ] תובנה והעמקה (חשיבה מסדר גבוה):
   - שאלות חשיבה מסדר גבוה (ניתוח, הנמקה, ראיות, מה ישתנה אם...).
   - שאלת התלמיד: "איך הגעתי לתשובה? ומה תומך בה?".
   - דוגמה להכלה (זמן שקט Think Time, תפקידים בקבוצה), דיפרנציאליות (רמות מורכבות), הערכה (מחוון נימוק וראיה).
   - משפט מעבר מחייב: "בדקנו את הרעיון והסברנו אותו; כעת נשתמש בו במשימה"

4. [ י ] יצירה ויישום (סדנה מעשית ודיפרנציאלית):
   - שאלת התלמיד: "איך אשתמש במה שלמדתי? ואילו כלים יעזרו לי?".
   - פירוט 3 מסלולים גמישים: מסלול נתמך (עם כרטיסיית שלבים ובנק מילים), מסלול עצמאי (יישום מלא), ומסלול העמקה ויצירתיות (אתגר חשיבתי).
   - שולחן הוראה ממוקדת (4-6 תלמידים עם המורה) וכרטיסיית "מפתח חזרה להבנה".
   - דוגמה להכלה, דיפרנציאליות, והערכה מעצבת.
   - משפט מעבר מחייב: "נבדוק כעת מה הצלחנו לעשות ומה ניקח להמשך"

5. [ ח ] חתימה וצידה לדרך (רפלקציה והעברת למידה):
   - מענה ל-5 שאלות רפלקציה (מה למדתי, מה הראיה, מה עזר, מה דורש תרגול, היכן איישם).
   - הגדרת הצידה לדרך המדויקת שלוקח התלמיד.
   - דוגמה להכלה, דיפרנציאליות, והערכה עצמית מסכמת.
   - סיום במשפט התלמיד המנחה: "אני יודע מה אני לומד, מוצא דרך להשתתף, מבקש עזרה כשצריך ומראה איך התקדמתי".
...
3. [ ت ] محطة تفكير وتبصّر (תובנה והעמקה):
   ...
   • مثال تطبيقي على الاحتواء والدمج (הכלה והשתلבות): وقت كافٍ للتفكير الفردي الصامت (Think Time)، توزيع أدوار متناوبة ومحددة في مجموعات التعلم (جامع الأدلة، المتحدث، مسجل الأفكار) لضمان مساهمة طالب الدمج بدور ملموس ومقدر.
...
4. [ ي ] محطة إنجاز وتطبيق (יצירה ויישום):
   ...
   • مثال تطبيقي على الاحتواء والدمج (הכלה והשתلבות): العمل ضمن طاولة التدريس المركّز برعاية المعلم في جو من الأمان والدعم دون أي وسم سلبي، الاستعانة بالزميل المساند، وحرية اختيار وسيلة التعبير والإنتاج (كتابي، مجسم، رسم تخطيطي، تسجيل صوتي).
...
5. [ ح ] محطة حصاد وزوّادة (חתימה וצידה לדרך):
   ...
   • مثال تطبيقي على الاحتواء والدمج (הכלה והשתلבות): إتاحة خيارات تعبير متنوعة عن الزوّادة (كتابة سطر واحد، إكمال جملة مفتوحة، أو تلوين مقياس إنجاز)، والتأكد من خروج كل طالب بكرامة وشعور حقيقي بالنجاح والإنجاز.
